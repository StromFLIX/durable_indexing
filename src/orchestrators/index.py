...
def index(context: DurableOrchestrationContext):
    ...
    while True:
        ...
        yield context.call_activity(name="ensure_index_exists", input_="test-index")
        task_list = []
        for blob_name in blob_list_result["blob_names"]:
            task_list.append(context.call_sub_orchestrator(name="index_document", input_={"blob_url": blob_name}, instance_id=context.new_uuid()))
...
def index_document(context: DurableOrchestrationContext):
    ...
    document = yield context.call_activity("document_cracking", url)
    chunks = yield context.call_activity("chunking", document)
    chunks_with_embeddings = yield context.call_activity("embedding", chunks)
    yield context.call_activity("add_documents", {"chunks": chunks_with_embeddings, "index_name": "test-index"})